...
export function KilroyRunViewer() {
  ...
  // The single node shown in red: the last failed node in history, but only when
  // the run has already ended in failure (during execution, all failed nodes are yellow).
  const terminalFailedNode = useMemo(() => {
    if (!runFailed) return undefined;
    let lastFailIdx = -1;
    stageHistory.forEach((v, i) => { if (v.status === "fail") lastFailIdx = i; });
    return lastFailIdx >= 0 ? stageHistory[lastFailIdx].node_id : undefined;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [stageHistory, runFailed]);
          ...
          {cycleMenuOpen && cycleNodes && cycleNodes.length > 0 && (
            <div className={`border-b ${runFailed ? "border-orange-800/20 bg-orange-950/20" : "border-yellow-800/20 bg-yellow-950/10"}`}>
              {cycleNodes.map((nodeId) => {
                ...
                const iconCls = visit.status === "pass" ? "text-green-400" : visit.status === "fail" ? "text-red-400" : visit.status === "interrupted" ? "text-amber-600" : "text-amber-400 animate-pulse";
        ...
        <div className="flex-1 min-w-0">
          {dot ? (
            <DotPreview
              ...
              terminalFailedNode={terminalFailedNode}
